## Edited from https://rdrr.io/cran/rpart/src/tests/usersplits.R
...
temp1 <- function(y, wt, parms) {
  ...
  list(label = pred, deviance = wsum - purity)
}
...
temp2 <- function(y, wt, x, parms, continuous) {
  ...
  if (continuous)
  {
    ...
    if (max(y) == 1)
      return(list(goodness = rep(0, n - 1), direction = rep(1, n - 1)))
    subtotal <- matrix(0, max(y), n)
    for (i in 1:nrow(subtotal))
      subtotal[i,] <- cumsum(wt * (y == i))
    left  <- subtotal[, -n]
    right <- subtotal[, n] - left
    if (n == 2)
      goodness <- sumw
    else
      goodness <- apply(left, 2, max) + apply(right, 2, max)
    list(goodness = goodness / sumw, direction = rep(1, n - 1))
  }
  else
    stop("??")
}
...
temp3 <- function(y, offset, parms, wt) {
  ...
  list(y=y, parms=0, numresp=1, numy=1,
       summary= function(yval, dev, wt, ylevel, digits ) {
         paste("  pred=", format(signif(yval, digits)),
               ", impurity=" , format(signif(dev/wt, digits)),
               sep='')
       })
}
...
temp1 <- function(y, wt, parms) {
  ...
  list(label = pred, deviance = entropy)
}
...
temp2 <- function(y, wt, x, parms, continuous) {
  ...
  if (continuous)
  {
    ...
    if (max(y) == 1)
      return(list(goodness = rep(0, n - 1), direction = rep(1, n - 1)))
    if (n == 2)
      goodness <- -sum(wt / sumw * log(wt / sumw))
    else
    {
      subtotal <- matrix(0, max(y), n)
      for (i in 1:nrow(subtotal))
      ...
      right <- subtotal[, n] - left
      sum1 <- cumsum(wt)[-n]
      sum2 <- rev(cumsum(rev(wt)))[-1] # to reduce the round-off error by subtraction
      left  <- left  / rep(sum1, each = max(y))
      right <- right / rep(sum2, each = max(y))
      goodness <- apply(left  * log(left ), 2, sum, na.rm = T) * sum1 +
                  apply(right * log(right), 2, sum, na.rm = T) * sum2
      goodness <- goodness - sum(subtotal[,n] * log(subtotal[,n] / sumw))
    }
...
elist <- list(eval = temp1, split = temp2, init = temp3)
rm(list = c("temp1", "temp2", "temp3"))
